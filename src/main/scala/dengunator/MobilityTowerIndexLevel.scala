...
object MobilityModel {
  def main(args: Array[String]) {
    val spark_host = "lasantha-GE40-2PC-Dragon-Eyes"
    val host = "localhost"
    val hdfs_port = 9000
    ...
    val cellIndexTowerIndexSchema = StructType(Array(
      ...
      StructField("TowerIndex", StringType, nullable = true)))

    val cellIndexTowerIndexDf = sqlContext.read
      .format("com.databricks.spark.csv")
      .option("header", "true") // Use first line of all files as header
      .schema(cellIndexTowerIndexSchema)
      .load("hdfs://" + host + ":" + hdfs_port + "/data/resources/cellid_TowerIndex_mapcellid_TowerIndex_map.csv").cache()
